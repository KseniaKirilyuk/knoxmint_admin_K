  ...
  const handleMultiImportFile = async (file) => {
    ...
    try {
      ...
      sheets.forEach(sheetName => {
        ...
        for (let pi = 1; pi < jsonData.length; pi++) {
          const pr = jsonData[pi] || []
          if (pr.some(c => (c != null ? String(c) : '').trim().toLowerCase() === 'coin cost')) {
            pricesHeaderIdx = pi; break
          }
        }
        ...
        if (priceRows.length > 1) {
          ...
          priceRows.slice(1).forEach(row => {
            ...
            if (!code) return
            prices[code] = { coinCost: pd(ccIdx>=0?row[ccIdx]:null), grading70: pd(g70Idx>=0?row[g70Idx]:null), grading69: pd(g69Idx>=0?row[g69Idx]:null), ungraded: pd(ugIdx>=0?row[ugIdx]:null) }
          })
        }
        parsedData[sheetName] = {
          coinCodes, contributions, prices,
